...
export function EditorPreviewCard({
  ...
  const previewAspectRatio = useMemo(() => {
    ...
    const ratio = ASPECT_RATIOS.find(
      (item) => item.value === adjustments.aspectRatio
    )?.ratio;
    if (ratio) {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">编辑预览</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="secondary" onClick={onCopy} disabled={!selectedAsset}>
            复制设置
          </Button>
          <Button
            size="sm"
      ...
      <div ref={containerRef} className="relative flex-1">
        <div
          className={cn(
            "relative flex h-full w-full items-center justify-center rounded-[28px] border border-white/10 bg-slate-950/60 touch-none",
        ...
        >
          {frameSize.width > 0 && frameSize.height > 0 && (
            <div
              className="relative overflow-hidden rounded-[24px] border border-white/10 bg-black/40 shadow-[0_20px_60px_rgba(0,0,0,0.55)]"
              style={{ width: frameSize.width, height: frameSize.height }}
            >
              <div
                className="h-full w-full"
                style={{
                  transform: `translate3d(${viewOffset.x}px, ${viewOffset.y}px, 0) scale(${viewScale})`,
