...
export const buildHistogram = (data: Uint8ClampedArray): HistogramData => {
  ...
  const strictMonochromeByPixelDelta =
    p95ChannelDelta <= MONOCHROME_P95_THRESHOLD && meanChannelDelta <= MONOCHROME_MEAN_THRESHOLD;
  ...
  const maxAbsBinDelta = Math.max(rgDistance.maxAbs, rbDistance.maxAbs, gbDistance.maxAbs);
