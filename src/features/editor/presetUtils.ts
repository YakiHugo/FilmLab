...
const SAVE_CUSTOM_PRESETS_DEBOUNCE_MS = 300;
let _saveCustomPresetsTimer: ReturnType<typeof setTimeout> | null = null;

export const saveCustomPresets = (presets: Preset[]) => {
  if (typeof window === "undefined") {
    return;
  }
  if (_saveCustomPresetsTimer) {
    clearTimeout(_saveCustomPresetsTimer);
  }
  _saveCustomPresetsTimer = setTimeout(() => {
    _saveCustomPresetsTimer = null;
    window.localStorage.setItem(CUSTOM_PRESETS_KEY, JSON.stringify(presets));
  }, SAVE_CUSTOM_PRESETS_DEBOUNCE_MS);
};
...
export const normalizeImportedPresets = (parsed: unknown): Preset[] => {
  ...
  return incoming.filter(isPresetLike).map((preset, index) => {
    ...
    return {
      ...
      description:
        typeof preset.description === "string" ? preset.description : fallbackDescription,
      adjustments: (preset.adjustments as Preset["adjustments"]) ?? {},
      filmProfileId: typeof preset.filmProfileId === "string" ? preset.filmProfileId : undefined,
