...
type CanvasSliderTone = "neutral" | "temperature" | "tint";
type CanvasSliderVariant = "default" | "canvasDock";

export interface CanvasSliderRowProps {
  label: string;
  value: number;
  min: number;
  max: number;
  ...
  tone?: CanvasSliderTone;
  variant?: CanvasSliderVariant;
...
export const CanvasSliderRow = memo(function CanvasSliderRow({
  label,
  value,
  min,
  max,
  ...
  const valueInputAriaLabel = `${label} value input`;
  const resetAriaLabel = `Reset ${label}`;
