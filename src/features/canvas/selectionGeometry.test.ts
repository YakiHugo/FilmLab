...
describe("selection geometry", () => {
  ...
  it("merges marquee hits into the base selection only for additive drags", () => {
    ...
    expect(mergeSelectionIds(["base-a"], ["hit-a", "hit-b"], false)).toEqual([
      "hit-a",
      "hit-b",
    ]);
  });
  ...
  it("uses the configured screen-space threshold before drag selection becomes active", () => {
    expect(
      selectionDistanceExceedsThreshold({ x: 20, y: 20 }, { x: 22, y: 23 }, 4)
    ).toBe(false);
    expect(
      selectionDistanceExceedsThreshold({ x: 20, y: 20 }, { x: 24, y: 23 }, 4)
    ).toBe(true);
  });
  ...
  it("converts screen-space bounds back into world-space using viewport and zoom", () => {
    expect(
      screenRectToWorldRect(
        { x: 260, y: 180, width: 320, height: 160 },
        { x: 100, y: 60 },
        2
      )
    ).toEqual({
