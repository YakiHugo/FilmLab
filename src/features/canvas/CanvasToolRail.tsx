...
const toolButtons = [
  ...
  { tool: "hand" as const, icon: Hand, label: "Pan" },
...
export function CanvasToolRail() {
  ...
  return (
    <div className="absolute left-3 top-[64px] z-20 flex w-11 flex-col items-center gap-0.5 rounded-2xl border border-white/10 bg-black/60 py-2 shadow-lg backdrop-blur-xl">
      {toolButtons.map((btn) => {
        const Icon = btn.icon;
          ...
          <div key={btn.tool} className="relative">
            <button
              ...
              className={cn(
                ...
                active ? "bg-white text-zinc-950" : "text-zinc-400 hover:bg-white/10 hover:text-zinc-200",
              )}
