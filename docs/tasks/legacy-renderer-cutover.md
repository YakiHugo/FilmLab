...
**Slice 6 implementation notes (done):**

The legacy CPU `buildAsciiCellGrids` work moved into a NEW two-stage compute chain on the GPU:

- `analysis.wgsl` (extended) — accumulates a 27-float feature vector AND a per-cell averaged RGBA into a new `cellColor` storage buffer in a single pass over each cell's source pixels. Slot 0 of `features` holds the raw cell luminance untouched (avoids the dispatch-internal race that would arise if toneNormalize wrote back to it while reading neighbours).
- `toneNormalize.wgsl` (new) — reads features[base+0] for self + L/R/U/D neighbours, applies brightness/contrast/density/coverage/invert/edgeEmphasis with the same ordering as the legacy CPU path, alpha-cutoff gates against `cellColor[i].a`, and writes the post-normalization tone into a NEW `cellTone` storage buffer. Floyd-Steinberg → Bayer 8×8 ordered (FS is sequential by design; Bayer keeps the dithered look without a sequential dependency, the ≤4/255 gate tolerates the substitution, project is pre-launch — we are not dual-pathing FS for legacy parity).
- `selection.wgsl` — small change: cell density now sourced from `cellTone[cellIdx]` (new binding 4) instead of `features[base+0]`. With a density-sorted charset, `structureWeight=0` density-distance matching reproduces the legacy `idx = round(tone * (n-1))` mapping. Other feature slots (sub-grid / edge histogram / centroid) unchanged.
- `composition.wgsl` (extended) — single fullscreen pass that renders ONE layer (`layerMode = 0` background or `1` foreground). Foreground covers glyph + dot modes, grayscale / full-color / duotone; background covers blurred-source canvas, solid fill, per-cell solid; grid overlay composited via `ascii_source_over` mirroring the legacy GLSL. Glyph index sourced from the selection buffer (no per-pixel `tone * (n-1)` recompute). Dot radius reads `cellTone` directly and un-inverts when `invert=true` (matches legacy behaviour: dots track brightness regardless of invert).

The new `passes/carrier/ascii/index.ts` adapter chains: analysis → toneNormalize → selection (one compute-only execute call) → optional `utility/gaussianBlur` h+v (separate execute) → composition[bg] → `utility/layerBlend` against base (separate executes; layerBlend can not bind a non-priorInput layer texture if the bg-composition output sits inside an executor chain) → composition[fg] → `utility/layerBlend` with EditorLayerBlendMode against current composite. `createPerDeviceCache` amortizes six pipeline caches + ShaderCache per device.

`asciiEffect.ts` lost `buildAsciiCellGrids` / `buildAsciiCarrierGpuInput` / `prepareCarrierGpuInput` / `parseHexColor` / `AsciiCellGrids` / `ALPHA_CUTOFF` / the `ASCII_CARRIER_SLOT_ID` constant; the surface call now passes `params: AsciiCarrierSurfaceParams` (with the tone-normalization fields directly).

Deleted: `src/lib/renderer/gpuAsciiCarrier.ts`, `shaders/AsciiCarrier.frag`, `shaders/templates/asciiCommon.glsl`, `PipelineRenderer.{renderAsciiCarrierComposite,renderAsciiCarrierLayer,renderAsciiBackgroundSourceLayer,uploadCellRgbaTexture,uploadCellR8Texture,getGlyphAtlas,pruneAsciiGlyphAtlasCache,releaseAsciiGlyphAtlasRecord}` + `asciiGlyphAtlasCache`/`asciiGlyphAtlasLru` fields + `emptyMaskTexture` field/constructor init/dispose + `AsciiCarrierGpuInput` import + `AsciiGpuCarrierInput`/`GlyphAtlasInput`/`AsciiBackgroundSourceInput`/`AsciiGlyphAtlasRecord`/`AsciiAtlasDebugSnapshot`/`AsciiLayerKind` types. `ProgramRegistry` lost the `asciiCarrier` program entry along with `ASCII_COMMON_MARKER`/`injectAsciiCommon`/`asciiCommonGlsl`/`asciiCarrierFragRaw` imports.

Tests: `asciiEffect.test.ts` rewired to mock `@/lib/gpu/passes/carrier/ascii` and assert the new params shape. `ProgramRegistry.test.ts` lost the asciiCarrier lazy-cache test (no longer applicable). Total tests: 134 files / 672 passed (was 673).

Smoke: existing `scripts/gpu-smoke/ascii.html` fixtures A/B/C still exercise the compute chain after the bind-group / uniform additions. Visual parity ≤ 4/255 vs production ASCII is deferred to a real-GPU run on user hardware (the legacy WebGL2 reference path is gone — re-establishing it in the smoke harness would require inlining the deleted GLSL + a parallel mini-renderer, which is heavier than s5's overlay parity setup since AsciiCarrier.frag depends on the asciiCommon.glsl helpers + a pre-built cell-color/tone texture upload pair). Light-touch closure matches s5.
