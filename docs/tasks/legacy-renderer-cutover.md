...
- `pnpm tsc --noEmit` clean, `pnpm vitest run` 674/674 pass (down from 682 — the 5 deleted localMaskShape renderer tests + 3 deleted ProgramRegistry tests for removed programs). `pnpm lint` baseline unchanged from main (only the pre-existing `passes/develop/curve.ts` empty-interface error). `pnpm dead-code` totals identical to baseline (19 unused files / 1 unused dep / 90 unused exports).
